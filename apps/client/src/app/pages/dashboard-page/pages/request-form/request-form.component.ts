...
import {filter, forkJoin, map, shareReplay, switchMap, takeUntil, tap} from 'rxjs';
import {
    Buyer,
    PayType,
    Product,
...
export class RequestFormComponent implements OnInit {
    ...
    readonly displayedControls = {
        ...
        count: this.userService.checkRole([UserRole.Manager, UserRole.Storekeeper]),
        ...
        address: this.userService.checkRole([UserRole.Manager, UserRole.Logistician]),
    ...
    readonly references$ = forkJoin([
        ...
        map(([buyers, products, providers, users, vehicles, vendors]) => ({
            ...
            responsibles: users.filter((u) => u.role === UserRole.Admin || u.role === UserRole.Manager).map((d) => ({id: d.id, label: d.label})),
        })),
